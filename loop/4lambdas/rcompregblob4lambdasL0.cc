...
#include "arcomp.h"
#include "arrscomp.h"
#include "cmatrixablob4lambdasL0.h"
#include "rcompsol.h"
...
ofstream myfile;
myfile.precision(16);
myfile.open("Fvsy4lambdasAFN3j0.dat");
double loopweightblob;
int L=3;



for(loopweightblob=0;loopweightblob<=1.5;loopweightblob=loopweightblob+0.1)
{
cout<<loopweightblob<<endl;
...
// Defining a complex matrix.
  ...
  ARrcCompStdEig<T> prob(csize, 4L);
...
cout << prob.Eigenvalue(3).real() <<endl;

double F1, F2, F3, F4;
F1=-(log(abs(prob.Eigenvalue(0).real())))/(2*L);
F2=-(log(abs(prob.Eigenvalue(1).real())))/(2*L);
F3=-(log(abs(prob.Eigenvalue(2).real())))/(2*L);
F4=-(log(abs(prob.Eigenvalue(3).real())))/(2*L);
