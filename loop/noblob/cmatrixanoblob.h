...
for(int i=(2*L-2); i>=0; i=i-2){					//loop over odd Xi (even when considering first position as position 1, not 0)
for (it=VTable.begin(); it!=VTable.end(); it++){		//loop over components of input vector


if (VTable[it->first].loop[i]==i+1){
WTable[it->first].vectcomp=x2*VTable[it->first].vectcomp+loopweight*VTable[it->first].vectcomp;
WTable[it->first].loop=VTable[it->first].loop;
}									//end of loop[i]=i+1 if statement
     ...
     for(int i=0; i<(sizeof(a)/sizeof(int));i++)
{
	...
	if (a[i]!=-1)
	total=(sizeof(a)/sizeof(int))*total+a[i];

}
                          
int totalmod;
totalmod=total%16787;   						//Change this depending on number of expected states                 
