...
#include "arcomp.h"
#include "arrscomp.h"
#include "cmatrixablobL0trial2.h"
#include "rcompsol.h"
...
ofstream myfile;
myfile.precision(16);
//myfile.open("EigenvaluesG5r1blob34567.dat");
myfile.open("test.dat");
double gamma;
int r1=2;
double loopweightblob;
int L;
//double loopweight=sqrt(2);


for(gamma=0.7;gamma<=0.7;gamma=gamma+0.025)
{
//cout << gamma << endl;
...
for(L=8;L<=8;L++)
{
cout<<"Beginning of L iteration"<<endl;
//double loopweight=loopweightblob;
...
// Defining a complex matrix.
  ...
  ARrcCompStdEig<T> prob(csize, 7L);
  ...
  while (!prob.ArnoldiBasisFound()) {
	...
	cout<<"about to multiply by transfer matrix"<<endl;
...
//myfile <<prob.Eigenvalue(0).real()<< "   "<<prob.Eigenvalue(1).real()<< "   "<<prob.Eigenvalue(2).real()<< "   "<<prob.Eigenvalue(3).real()<< "   "<< "   "<<prob.Eigenvalue(4).real();
