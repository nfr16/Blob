...
for(loopweight=1;loopweight<=1;loopweight=loopweight+0.1)  
{
...
// Defining a complex matrix.
  ...
  ARrcCompStdEig<T> prob(csize, 3L);
...
double F1,F2,F3,F4;
F2=-(log(abs(prob.Eigenvalue(2).real())))/(2*L);
F3=-(log(abs(prob.Eigenvalue(1).real())))/(2*L);
F4=-(log(abs(prob.Eigenvalue(0).real())))/(2*L);
if(L==3)
myfile << loopweight <<"  ";

myfile << "   "<<F2<<"   "<<F3<< "   "<<F4<<endl;
